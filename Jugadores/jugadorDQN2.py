...
class FusionAgenteDQN(Jugador):
    ...
    def __init__(self, nombre, color, mision, alpha=0.1, gamma=0.9, epsilon=0.1, epsilon_decay=0.995, epsilon_min=0.01):
    ...
    def create_multi_output_model(self):
        ...
        # Salidas
        reforzar_territorio = Dense(42, activation='softmax', name='reforzar_territorio')(x)
        num_tropas_reforzar = Dense(1, activation='linear', name='num_tropas_reforzar')(x)
        territorio_origen_atacar = Dense(42, activation='softmax', name='territorio_origen_atacar')(x)
        territorio_destino_atacar = Dense(42, activation='softmax', name='territorio_destino_atacar')(x)
        territorio_origen_mover = Dense(42, activation='softmax', name='territorio_origen_mover')(x)
        territorio_destino_mover = Dense(42, activation='softmax', name='territorio_destino_mover')(x)
        num_tropas_mover = Dense(1, activation='linear', name='num_tropas_mover')(x)
 ...
 # ----------------------- Otros métodos específicos del juego -----------------------
    ...
    def aplicar_accion_reforzar(self, accion_vector, tablero):
        ...
        if not self.es_accion_valida(accion, tablero):
            ...
            return -100, self.obtener_estado_siguiente("reforzar", tablero)
        ...
        if accion['type'] == 'fortificar':
            ...
            recompensa += 5 * accion['tropas'] if self.es_territorio_vulnerable(pais, tablero) else -2 * accion['tropas']
    ...
    def aplicar_accion_atacar(self, accion_vector, tablero):
        ...
        if not self.es_accion_ataque_valida(accion, tablero):
            ...
            return -100, self.obtener_estado_siguiente("atacar", tablero)
        ...
        if accion['type'] != 'pass':
            ...
            if pais_destino.jugador == self:
                recompensa += 10 if jugador_destino_anterior != self else -2 * pais_origen.tropas
    ...
    def aplicar_accion_mover_tropas(self, accion_vector, tablero):
        ...
        if not self.es_accion_mover_tropas_valida(accion, tablero):
            ...
            return -100, self.obtener_estado_siguiente("mover_tropas", tablero)
        ...
        if accion['type'] != 'pass':
            ...
            recompensa += 5 if self.es_territorio_vulnerable(pais_destino, tablero) else -3
    ...
    def actualizar_vector_accion(self, action_vectors, tipo, territorio, tropas=None):
        ...
        if tropas is not None:
            if tipo == 'reforzar_territorio':
                key = 'num_tropas_reforzar'
            else:
                key = 'num_tropas_' + tipo.split('_')[-1]  # Tomamos la última palabra
            action_vectors[key][0] = tropas
        return action_vectors
